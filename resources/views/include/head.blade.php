    ...
    <div class="container-fluid">
        ...
        <div class="collapse navbar-collapse">
            ...
            <ul class="nav navbar-nav navbar-right">
                    ...
                    <li class="dropdown">
                        ...
                        <ul class="dropdown-menu">
                            ...
                            @if ($user->account_type == User::ACCOUNT_TYPE_CREATOR)
                                <li><a href="<?= URL::to('/role') ?>"><span class="glyphicon glyphicon-warning-sign"
                                            aria-hidden="true"></span>&nbsp Role</a></li>
                            @endif
                ...
                @if (
                    $user->account_type == User::ACCOUNT_TYPE_CREATOR ||
                        $user->account_type == User::ACCOUNT_TYPE_ADMIN ||
                        $user->account_type == User::ACCOUNT_TYPE_TEACHER)
                    <li class="dropdown">
                        <a href="#" class="dropdown-toggle" data-toggle="dropdown">
                ...
                @if ($user->account_type != User::ACCOUNT_TYPE_CREATOR)
                    ...
                    <li class="dropdown">
                        <a href="#" class="dropdown-toggle" data-toggle="dropdown">
                            ...
                            <?php
                            $data_notif = UserNotification::where('user_id', $user->id)
                                ->orderBy('created_at', 'DESC')
                                ->limit(5)
                                ->get();
                            $count_notif = UserNotification::where('user_id', $user->id)
                            ...
                            <ul class="dropdown-menu">
                                @if ($data_notif != null)
                                    @foreach ($data_notif as $notif)
                                        <li><a style="@if ($notif->status != UserNotification::STATUS_READ) color: blue @endif"
                                                onclick="showNotif('{{ $notif->notification_id }}')">{{ $notif->getNotification->notification_title }}</a>
                                        </li>
                                    @endforeach
                                @endif
                            </ul>
